...
const CampoModal: React.FC<CampoModalProps> = ({ open, handleClose, onSave, clienteId }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/campos/${clienteId}`, {
        method: 'POST',
        headers: {
      ...
      NotificationService.error(error instanceof Error ? error.message : 'Error al crear el campo');
    } finally {
      setCampoNombre('');
      setCampoUbicacion('');
      handleClose();
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      ...
      >
        ...
        <TextField fullWidth margin='normal' label='Nombre' value={campoNombre} onChange={(e) => setCampoNombre(e.target.value)} />
        <TextField fullWidth margin='normal' label='Ubicación' value={campoUbicacion} onChange={(e) => setCampoUbicacion(e.target.value)} />
